				...
				for input_size in ${input_size_array[@]}; do
					for format in ${format_array[@]}; do
						for agg in ${agg_array[@]}; do
							python prepare_main.py --exp_name 'HD' --path './sample_data/helpdesk2.csv' --agg 'avg' --measure 'sojourn' --fv_format $format --start "2011-01-01 00:00:00" --end "2012-06-30 23:59:59" --range "0,0,0,"$r_hour",0,0" --stride "0,0,0,"$stride_hour",0,0" --input_size $input_size --output_size 1 --edge_threshold 0 --node_threshold 100
							python train_main.py --search 'True' --fv_format '2d' --prediction $pred --exp_id "HD_avg_sojourn_2d_2011-01-01-2011-01-01_range0,0,0,"$r_hour",0,0_stride0,0,0,"$stride_hour",0,0_input"$input_size"_output1_0_100" --algo 'None' --result $result
							if [ $format = "2d" ]; then
