						...
						for agg in ${agg_array[@]}; do
							...
							if [ $format = "2d" ]; then
								for algo in ${algo_array[@]}; do
									python train_main.py --search $search --fv_format '2d' --prediction $pred --exp_id "HOS_avg_"$measure"_2d_2018-01-01-2018-01-01_range0,0,0,"$r_hour",0,0_stride0,0,0,"$stride_hour",0,0_input"$input_size"_output1_100_100" --algo $algo --result $result
								done;
							else
								python train_main.py --fv_format '3d' --prediction $pred --exp_id "HOS_avg_"$measure"_3d_2018-01-01-2018-01-01_range0,0,0,"$r_hour",0,0_stride0,0,0,"$stride_hour",0,0_input"$input_size"_output1_100_100" --algo 'basic_LRCN' --result $result
							fi
						done;
					done;
				done;
